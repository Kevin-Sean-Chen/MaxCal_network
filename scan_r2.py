...
def LIF_firing(synaptic_weights, noise_amp, syn3_tau=5, lt=100000):
    ...
    tau_synaptic = np.array([5, 5, syn3_tau])  #5.0  # synaptic time constant
...
# w_s = np.array([5,30,60,90])  # for delay
...
for ww in range(len(w_s)):
    for nn in range(len(n_s)):
        ...
        signs[ww,nn] = corr_param(true_s, inf_w, 'binary')
        coss[ww,nn] = cos_ang(inf_w, true_s)
...
# %%
plt.figure()
img = plt.imshow(signs, aspect='auto',  origin='lower');
