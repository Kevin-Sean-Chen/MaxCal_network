# -*- coding: utf-8 -*-
"""
Created on Mon Sep 16 23:05:14 2024
...
# %%
### adding synaptic delay
# using synaptic filtering or time delay in spikes
...
tau_synaptic = np.array([5, 100, 5])  #5.0  # synaptic time constant
delayed_coupling = np.array([0, 0, 0])
t_offset = np.max(delayed_coupling)
...
# Simulation loop
for t in range(t_offset+1, timesteps):
    ...
    spike_indices = np.where(v_neurons[:, t - t_offset] > v_threshold)[0]
    ...
    for ii in range(N):
        if ii == 1:
            v_neurons[:, t] += synaptic_inputs[:, t  - 0*t_offset ]*dt  # for the neuron with delay
        else:
            v_neurons[:, t] += synaptic_inputs[:, t  - 0 ]*dt
...
plt.figure()
for tt in range(timesteps-1 - t_offset):
...
def spk2statetime(firing, window, N=N, combinations=combinations):
    ...
    for tt in range(lt-window - t_offset):
