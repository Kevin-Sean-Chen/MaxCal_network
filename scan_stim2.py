...
# %% scan stimulus configuration
# given the EI network, at a specific weight regem, scan stim params

# %%
def LIF_firing(synaptic_weights, noise_amp, syn_delay=None, syn_ratio=None, stim=None, stim_inter=100, stim_dur = .5, counter=0):
    """
...
# %% scanning loop
pert_j = 0 ### choose the perturbed regeme
...
R2s = np.zeros((3, len(n_s), reps, 2))  ### 3 motifs x scan-params x repeats x stim-condition
signs = R2s*0
coss = R2s*0
...
for rr in range(reps): ### repears
    for ss in range(0,2):  ### stim condition
        for ww in range(0,1):  ### motifs
            Wij = Ws[ww]  # asign motif
            for ii in range(len(stim_params)):  ### scan stim params
                print(ww); print(ii)
                S = Wij* w_s[pert_j]  # *fault_w[ww] #
                # firing = LIF_firing(S, fault_n[ww]*1+w_s[pert_j]*0, syn_delay=None, syn_ratio=None, stim=stim_conds[ss], 
                                    # stim_inter=stim_params[ii])  ### tune noise, delay, or ratio
                firing = LIF_firing(S, fault_n[ww]*1+w_s[pert_j]*0, syn_delay=None, syn_ratio=None, stim=stim_conds[ss], 
                                    stim_dur=stim_params[ii], stim_inter=100)  ### tune noise, delay, or ratio
...
for ii in range(0,1):  ### pick one motif
    for ss in range(2):  ### stim condition
        ...
        plt.xlabel('stim duration',fontsize=20)
        plt.subplot(133); 
